...
static void post_read_hook(THREADID tid, syscall_ctx_t *ctx) {
  ...
  // {
  //   char arg[2];
  //   memcpy(arg, (char*)buf, 2);
  //   memcpy(arg+1, "\0", 1);
  //   printf("\tread(0, 0x%02x, 1)\n", arg[0]);
  // }
  
  if(fd == 0)
  {
    char * args = (char *)malloc(count+1);
    memcpy(args, (char*)buf, count);
    memcpy(args+count, "\0", 1);
    // printf("\tread(0, %s, %d)\n", args, (int)count);
  }
  ...
  if (is_fuzzing_fd(fd)) {
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      tagmap_setb(buf + i, t);
      // printf("\t%s[read] %d, lb: %d,  %s%s\n", KYEL, i, t, tag_sprint(t).c_str(), KNRM);
